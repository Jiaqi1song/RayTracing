...
public:
    ...
    __device__ virtual bool hit(const ray &r, const interval &ray_t, hit_record &rec, curandState *state, StaticStack<T, N>& stack) const = 0;
...
public:
    ...
    __device__ bool hit(const ray &r, const interval &ray_t, hit_record &rec, curandState *state, StaticStack<T, N>& stack=nullptr) const override
    {
        ...
        for (int i = 0; i < obj_num; ++i)
        {
            if (objects[i]->hit(r, interval(ray_t.min, closest_t), tmp_rec, state, stack))
            {
                hit_anything = true;
                closest_t = tmp_rec.t;
                rec = tmp_rec;
