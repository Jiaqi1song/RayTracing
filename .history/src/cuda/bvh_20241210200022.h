...
public:
    ...
    __device__ bool hit(const ray& r, const interval& ray_t, hit_record& rec, curandState* state, StaticStack<hittable*, 16> *stack) const override {
        ...
        while (!stack->empty()) {
            ...
            if (current->is_bvh) {
                ...
                if (current->hit(r, closest_so_far, rec, state, stack)) {
                    hit_anything = true;
