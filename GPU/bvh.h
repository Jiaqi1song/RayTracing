...
{
  public:
    __device__ DynamicStack(T *data, int capacity) : capacity_(capacity), size_(0) { data_ = data; }

    __device__ ~DynamicStack() { free(data_); }
...
class bvh_node : public hittable
{
  public:
    ...
    __device__ bool hit(const ray &r, const interval &ray_t, hit_record &rec,
                        StaticStack<hittable *, 16> *stack) const override
    {
        ...
        while (!stack->empty())
        {
            ...
            {
                if (current->hit(r, closest_so_far, rec, stack))
                {
                    hit_anything = true;
    ...
    __device__ friend hittable *build_bvh_node(hittable **objects, BVH_Node *bvh_data, size_t object_count,
                                               curandState *state);
